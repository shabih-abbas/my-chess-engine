...
int checkresult(BOARD *pos){
    if(pos->fiftyMoves > 100){
        printf("1/2-1/2 (fifty move rule (claimed by the engine))\n");
        return TRUE;
    }
    if(ThreeFoldRep(pos) >= 2){
        printf("1/2-1/2 (3-fold repetition (claimed by the engine))\n");
        return TRUE;
    }
    if(DrawMaterial(pos) == TRUE){
        printf("1/2-1/2 (insufficient material (claimed by the engine))\n");
        return TRUE;
    }
    MOVELIST list[1];
    ...
    int InCheck = SqAttacked(pos->KingSq[pos->turn], pos->turn ^ 1, pos);
    if(InCheck == TRUE){
        if(pos->turn == WHITE){
            printf("0-1 (black mates (claimed by the engine))\n");
            return TRUE;
        } else {
            printf("1-0 (white mates (claimed by the engine))\n");
            return TRUE;
        }
    } else {
        printf("\n1/2-1/2 (stalemate (claimed by the engine))\n");
        return TRUE;
    }
    return FALSE;
}
...
void XBoard_Loop(BOARD *pos, SEARCHINFO *info){
    ...
    while(TRUE){
        ...
        if(pos->turn == engineSide && checkresult(pos) == FALSE){
            info->starttime = GetTimeMs();
            info->depth = depth;

            if(time != -1){
                info->timeset = TRUE;
                time /= movestogo[pos->turn];
                time -= 50;
            ...
            printf("time:%d start:%d stop:%d depth:%d timeset:%d movestogo:%d mps:%d\n", time, info->starttime, info->stoptime, info->depth, info->timeset, movestogo[pos->turn], mps);
            SearchPosition(pos, info);
            if(mps != 0){
                movestogo[pos->turn ^ 1]--;
                if(movestogo[pos->turn ^ 1] < 1){
                    movestogo[pos->turn ^ 1] = mps;
                }
            }
        }
        fflush(stdout);
...
void Console_Loop(BOARD *pos, SEARCHINFO *info){
    printf("Welcome to My Chess Engine in Console Mode!\n");
