...
int ParseFen(char *fen, BOARD *pos){
    ...
    for(i = 0; i < 4; i++){
        ...
        switch(*fen){
            case 'K': pos->castlePerm |= WKSC; break;
            case 'Q': pos->castlePerm |= WQSC; break;
            case 'k': pos->castlePerm |= BKSC; break;
            case 'q': pos->castlePerm |= BQSC; break;
            default: break;
        }
        fen++;
    }
...
void ResetBoard(BOARD *pos) {
    ...
    for(index = 0; index < BRD_SQRS; ++index){
        pos->squares[index] = OFFBOARD;
    }

    for(index = 0; index < 64; ++index){
        pos->squares[SQ120(index)] = EMPTY;
    }
...
extern void PrintBoard(const BOARD *pos){
    ...
    printf("castle:%c%c%c%c\n",
            pos->castlePerm & WKSC ? 'K' : '-',
            pos->castlePerm & WQSC ? 'Q' : '-',
            pos->castlePerm & BKSC ? 'k' : '-',
            pos->castlePerm & BQSC ? 'q' : '-'
        );
    printf("PosKey:%llX\n", pos->posKey);
}